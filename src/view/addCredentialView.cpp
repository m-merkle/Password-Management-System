...
addCredentialView::addCredentialView(const std::string& userID, Database& db) : userID(userID), db(db)
{
    CredentialForm();
...
addCredentialView::addNewCredential()
{
    ...
    if(idcheck==false||username.length()==0||password.length()==0||email.length()==0||credname.length()==0)
